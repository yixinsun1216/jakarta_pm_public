...
\begin{figure}[H]
  ...
  \caption{\textbf{Figure 2: Infiltration Factor from Ambient Outdoor to Indoor Homes.}
  (a) Infiltration factors across regression specifications, including the new 24-hour aggregate model.
  ...
  (b) Heterogeneity in infiltration factors by household characteristics (income moved to Figure 4).
...
\begin{figure}[H]
  ...
  indoor smoking).
  (c) Infiltration factor heterogeneity by income quartile (moved from Figure 2).
...
\begin{figure}[H]
  ...
  \caption{\textbf{Figure A1: Infiltration Factor with Lagged Outdoor PM$_{2.5}$.}
  Distributed-lag specification showing infiltration $\beta_k$ coefficients for lags $k = 0, \ldots, 11$ hours,
  across three fixed-effect specifications. The cumulative infiltration factor (sum of $\beta_k$) is annotated.}
  \label{fig:A1}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{\textbf{Figure A2: Infiltration Factor at Different Aggregation Levels.}
  Infiltration factors estimated at 1-hour, 8-hour, and 24-hour averaging periods, across multiple
...
\section{Key Results Text}
\input{tables/paper_stats.tex}

Decomposition of indoor PM$_{2.5}$ sources has been completed and documented using
the mean-contribution and income-gap decomposition results shown in Figure 3,
Figure 4, and the associated tables.

\newpage
\input{tables/mean_contribution_table.tex}
...
\begin{table}[H]
  ...
  \input{tables/sensor_selection.tex}
...
\begin{table}[H]
  ...
  \input{tables/reg_outdoor_burning.tex}
  \label{tab:outdoor_burning}
\end{table}

\newpage
\input{tables/spike_contribution_table.tex}
